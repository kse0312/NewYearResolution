...
public class GetThread implements Runnable{
    ...
    public void run() {
        try(DataOutputStream dout = new DataOutputStream(socket.getOutputStream())){
            System.out.println("[Server Thread : DataOutputStream Ready]");
            ...
            if(file.exists()){
                ...
                dout.writeBytes("\r\n");
                dout.flush();
                System.out.println("[Server Thread : Print Web Page]");
            }else{
                System.out.println("[Server Thread : RequestFile is not Exist]");
            }
        } catch (IOException e) {
